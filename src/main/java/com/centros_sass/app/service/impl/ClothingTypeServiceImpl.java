...
import com.centros_sass.app.mapper.ClothingTypeMapper;
import com.centros_sass.app.model.catalogs.dynamic.belongings.ClothingType;
import com.centros_sass.app.repository.ClothingTypeRepository;
...
public class ClothingTypeServiceImpl implements ClothingTypeService {
    ...
    public Page<ClothingTypeResponseDTO> findAll(Pageable pageable) {
        return clothingTypeRepository.findAllByIsActiveTrue(pageable)
                .map(clothingTypeMapper::toResponse);
    }

    @Override
    @Transactional(readOnly = true)
    public Page<ClothingTypeResponseDTO> findAllInactive(Pageable pageable) {
        return clothingTypeRepository.findAllByIsActiveFalse(pageable)
                .map(clothingTypeMapper::toResponse);
    }

    @Override
    @Transactional(readOnly = true)
    public Page<ClothingTypeResponseDTO> findAllIncludingInactive(Pageable pageable) {
    ...
    @Override
    @Transactional
    public Optional<ClothingTypeResponseDTO> delete(Integer id) {
        return clothingTypeRepository.findById(id).map(existing -> {
