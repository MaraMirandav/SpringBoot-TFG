...
public class ReturnReasonController {
    ...
    public ResponseEntity<ApiDataResponse<List<ReturnReasonResponseDTO>>> findAll(
        ...
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Razones de devolución activas encontradas",
                page.getContent(),
                HttpStatus.OK.value(),
    ...
    @GetMapping("/inactive")
    public ResponseEntity<ApiDataResponse<List<ReturnReasonResponseDTO>>> findAllInactive(
            @PageableDefault(size = 20) Pageable pageable) {
        Page<ReturnReasonResponseDTO> page = returnReasonService.findAllInactive(pageable);
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Razones de devolución inactivas encontradas",
                page.getContent(),
                HttpStatus.OK.value(),
                (int) page.getTotalElements()));
    }
    ...
    @GetMapping("/all")
    public ResponseEntity<ApiDataResponse<List<ReturnReasonResponseDTO>>> findAllIncludingInactive(
            @PageableDefault(size = 20) Pageable pageable) {
        Page<ReturnReasonResponseDTO> page = returnReasonService.findAllIncludingInactive(pageable);
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Todas las razones de devolución encontradas",
                page.getContent(),
                HttpStatus.OK.value(),
                (int) page.getTotalElements()));
    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiDataResponse<ReturnReasonResponseDTO>> delete(@PathVariable Integer id) {
        ReturnReasonResponseDTO deleted = returnReasonService.delete(id)
                .orElseThrow(() -> new ResourceNotFoundException("ReturnReason", "id", id));
        return ResponseEntity.ok(new ApiDataResponse<>(
