...
public class ObjectTypeController {
    ...
    public ResponseEntity<ApiDataResponse<List<ObjectTypeResponseDTO>>> findAll(
        ...
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Tipos de objeto activos encontrados",
                page.getContent(),
                HttpStatus.OK.value(),
    ...
    @GetMapping("/inactive")
    public ResponseEntity<ApiDataResponse<List<ObjectTypeResponseDTO>>> findAllInactive(
            @PageableDefault(size = 20) Pageable pageable) {
        Page<ObjectTypeResponseDTO> page = objectTypeService.findAllInactive(pageable);
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Tipos de objeto inactivos encontrados",
                page.getContent(),
                HttpStatus.OK.value(),
                (int) page.getTotalElements()));
    }
    ...
    @GetMapping("/all")
    public ResponseEntity<ApiDataResponse<List<ObjectTypeResponseDTO>>> findAllIncludingInactive(
            @PageableDefault(size = 20) Pageable pageable) {
        Page<ObjectTypeResponseDTO> page = objectTypeService.findAllIncludingInactive(pageable);
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Todos los tipos de objeto encontrados",
                page.getContent(),
                HttpStatus.OK.value(),
                (int) page.getTotalElements()));
    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiDataResponse<ObjectTypeResponseDTO>> delete(@PathVariable Integer id) {
        ObjectTypeResponseDTO deleted = objectTypeService.delete(id)
                .orElseThrow(() -> new ResourceNotFoundException("ObjectType", "id", id));
        return ResponseEntity.ok(new ApiDataResponse<>(
                "Tipo de objeto eliminado",
                deleted,
                HttpStatus.OK.value()));
    }
